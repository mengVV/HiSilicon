...
#define INFANT_FRM_WIDTH    608 
#define INFANT_FRM_HEIGHT   608
...
static HI_VOID CnnTrashClassifyAiProcess(VIDEO_FRAME_INFO_S frm)
{
    ...
    if (GetCfgBool("trash_classify_switch:support_trash_classify", true)) {
        if (g_workPlug.model == 0) {
            ...
            HI_ASSERT(g_aicMediaInfo.osds);
            ret = CnnTrashClassifyLoadModel(&g_workPlug.model, g_aicMediaInfo.osds);//load is outside
            if (ret < 0) {
                g_workPlug.model = 0;
        ...
        ret = CnnTrashClassifyCal(g_workPlug.model, &frm, resFrm);//cal is outside
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, TRASH_RELEASE,
...
static HI_VOID* GetVpssChnFrameHandClassify(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    HI_S32 s32MilliSec = 200;//200ms取1帧，并送AI推理
    VO_LAYER voLayer = 0;
    VO_CHN voChn = 0;
...
//infant1
static HI_VOID InfantClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    if (GetCfgBool("infant_detect_switch:support_infant_classify", true)) {
        if (g_workPlug.model == 0) {
            ret = InfantDetectLoad(&g_workPlug.model);//在scenario里实现，参考Yolov2HandDetectResnetClassifyLoad
            if (ret < 0) {
                g_workPlug.model = 0;
                SAMPLE_CHECK_EXPR_GOTO(ret < 0, INFANT_RELEASE,
                    "load infant detect model err, ret=%#x\n", ret);
            }
        }

        VIDEO_FRAME_INFO_S resizeFrm;
        ret = MppFrmResize(&frm, &resizeFrm, INFANT_FRM_WIDTH, INFANT_FRM_HEIGHT);
        ret = InfantDetectCal(g_workPlug.model, &resizeFrm, &frm);//在scenario里实现
        SAMPLE_CHECK_EXPR_GOTO(ret < 0, INFANT_RELEASE,
            "infant classify plug cal FAIL, ret=%#x\n", ret);

        ret = HI_MPI_VO_SendFrame(voLayer, voChn, &frm, 0);
        SAMPLE_CHECK_EXPR_GOTO(ret != HI_SUCCESS, INFANT_RELEASE,
            "HI_MPI_VO_SendFrame fail, Error(%#x)\n", ret);
...
//infant2
static HI_VOID* GetVpssChnFrameInfantDetect(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        InfantClassifyAiProcess(frm, voLayer, voChn);
    }
...
static HI_S32 InfantClassifyAiThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "AIProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameInfantDetect, NULL);
...
//infant4 cp from hand4
static HI_S32 PauseDoUnloadInfantClassifyModel(HI_VOID)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if (GetCfgBool("infant_classify_switch:support_infant_classify", true)) {
        if (memset_s(&g_workPlug, sizeof(g_workPlug), 0x00, sizeof(g_workPlug)) != EOK) {
            HI_ASSERT(0);
        }
        /* when pause operation, should unload model */
        s32Ret = InfantDetectUnload(g_workPlug.model);
        SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "unload infant model err:%x\n", s32Ret);
        ConfBaseExt();
        g_num = 0;
    }
...
//infant5 main interface
HI_S32 SAMPLE_MEDIA_INFANT_DETECT(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
    ...
    /* create work thread to run ai */
    s32Ret = InfantClassifyAiThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "ai proccess thread creat fail:%s\n", strerror(s32Ret));
    ...
    pthread_join(g_aiProcessThread, NULL);
    g_aiProcessThread = 0;
    PauseDoUnloadInfantClassifyModel();
